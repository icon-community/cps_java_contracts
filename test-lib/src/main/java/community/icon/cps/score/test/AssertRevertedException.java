...
package community.icon.cps.score.test;
...
public class AssertRevertedException {
    ...
    public static UserRevertedException assertUserReverted(int expected, Executable executable,
                                                           Function<UserRevertedException, String> messageSupplier) {
        return assertUserReverted(new UserRevertedException(expected), executable, messageSupplier);
    }

    public static <T extends UserRevertedException> UserRevertedException assertUserReverted(T expected,
    ...
    public static <T extends UserRevertException> UserRevertedException assertUserRevert(T expected,
                                                                                         Executable executable,
                                                                                         Function<UserRevertedException, String> messageSupplier) {
    ...
    public static UserRevertedException assertUserRevertedFromJsonrpcError(int expected, Executable executable,
                                                                           Function<UserRevertedException, String> messageSupplier) {
    ...
    public static <T extends UserRevertedException> UserRevertedException assertUserRevertedFromJsonrpcError(T expected, Executable executable, Function<UserRevertedException, String> messageSupplier) {
        ...
        UserRevertedException e = new UserRevertedException(-1 * ((int) rpcError.getCode() + 30032),
                rpcError.getMessage());
        assertEquals(expected.getCode(), e.getCode(), messageSupplier == null ?
                () -> String.format("assertUserRevertedFromJsonrpcError expected: %d, actual: %d", expected.getCode()
                        , e.getCode()) :
                () -> messageSupplier.apply(e));
        return e;
    }

    public static <T extends UserRevertException> UserRevertedException assertUserRevertFromJsonrpcError(T expected,
                                                                                                         Executable executable, Function<UserRevertedException, String> messageSupplier) {
        return assertUserRevertedFromJsonrpcError(expected.getCode(), executable, messageSupplier == null ?
                (e) -> String.format("assertUserRevertFromJsonrpcError expected:%s, actual: %s", expected,
                        e.toString()) :
                messageSupplier);
    }
}
